...
class LoginView(APIView):
        ...
        try:
            ...
            if not user.is_active:
                if not user.email_verified_at:
                    ...
                    self.send_verification_email(user, token)
                    raise ValidationError({
                        'error': 'Account is not verified. A new verification email has been sent.'
                    })
                raise ValidationError({
    ...
    def update_user_login_info(self, user: AbstractBaseUser, request: Request) -> None:
        ...
        user.save(update_fields=[
            'last_login', 
            'last_active_at',
            *(['last_login_ip'] if hasattr(user, 'last_login_ip') else []),
            *(['last_login_user_agent'] if hasattr(user, 'last_login_user_agent') else []),
            *(['login_count'] if hasattr(user, 'login_count') else [])
        ])
